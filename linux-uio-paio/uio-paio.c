        ...
        for (off=0; i<argc; i++, off++) {
            ...
            if (cmd == CMD_WB) {
                ...
                *p = (uint16_t)(value & 0xFFFFu);
                printf("WW 0x%08lX: 0x%04X <- 0x%04X"NL, phys_addr+ (off*byte_width), prev, *p);
            } else if (cmd == CMD_WD) {
                uint32_t * p = ((uint32_t*)va) + off;
                uint32_t prev = *p;
                *p = (uint32_t)(value & 0xFFFFFFFFul);
                printf("WD 0x%08lX: 0x%08X <- 0x%08X"NL, phys_addr+ (off*byte_width), prev, *p);
            } else if (cmd == CMD_WQ) {
                uint64_t * p = ((uint64_t*)va) + off;
                uint64_t prev = *p;
                *p = (uint64_t)(value & 0xFFFFFFFFFFFFFFFFull);
                printf("WQ 0x%08lX: 0x%016lX <- 0x%016lX"NL, phys_addr+ (off*byte_width), prev, *p);
            }
        }
    }
...
static int usage(int argc, const char * argv[]) {
    ...
    printf(
            ...
            ""NL
            "memory I/O via /dev/mem or UIO."NL
            ""NL
